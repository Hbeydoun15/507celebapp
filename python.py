...
# ——— Page config ———————————————
st.set_page_config(page_title="🎤  Social Media Fame and accolades ", page_icon="🚀", layout="centered")
...
@st.cache_data
def load_data():
    insta   = pd.read_csv(os.path.join(BASE_DIR, "top_insta_influencers_data.csv"), low_memory=False)
    spotify = pd.read_csv(os.path.join(BASE_DIR, "Final database.csv"),            low_memory=False)
...
spotify.rename(columns={"Artist":"artist_name","Artist_followers":"monthly_listeners"}, inplace=True)
grammy.rename(columns={"artist":"artist_name"}, inplace=True)

# Extract primary
spotify["artist_name"] = spotify["artist_name"].astype(str).apply(
    lambda s: re.split(r"\s*(?:&|feat\.?|featuring)\s*", s, flags=re.IGNORECASE)[0].strip()
)
spotify["monthly_listeners"] = pd.to_numeric(spotify["monthly_listeners"], errors="coerce").fillna(0)
...
df = (insta[["artist_name","centrality"]]
...
# ——— UI header ———————————————
st.markdown("<h1 style='text-align:center;'>🚀 UpNext Dashboard</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center;'>Discover emerging talent with social & career insights</p>", unsafe_allow_html=True)
st.markdown("---")
...
fmt = FuncFormatter(lambda x, pos: (
    f"{x*1e-6:.1f}M" if x>=1e6 else
    f"{x*1e-3:.1f}k" if x>=1e3 else
    f"{int(x)}"
))
...
def show_leaderboard(df_lead, name_col, val_col, title, fmt):
    ...
    st.dataframe(
        disp
          .rename(columns={name_col: "Artist", val_col: title.replace(" ", "_")})
          .reset_index(drop=True)
    )

comma_fmt = lambda x: f"{int(x):,}"

show_leaderboard(top_followers, "artist_name", "centrality", "Instagram Followers", comma_fmt)
